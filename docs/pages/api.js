...
export async function getStaticProps({ params }) {
  const componentId = uppercaseFirst(kebapToCamelCase(params.id));

  try {
    const api = await import(`../static/api/${componentId}.json`);
...
export async function getStaticPaths() {
  const componentIds = await import('../static/api/__ids__.json');
  const paths = componentIds.map(id => `/api/${id}`);
